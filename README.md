...
**提问 → 基于资料的可溯源回答 ｜ 无关问题阈值拒答**

<p align="center">
  <img src="docs/screenshots/ask.png" width="49%" alt="提问与可溯源回答" />
  <img src="docs/screenshots/refuse.png" width="49%" alt="无关问题阈值拒答" />
</p>

**资料管理（上传 / 替换 / 删除，可见文件名、chunk 数、大小）**

<p align="center">
  <img src="docs/screenshots/docs.png" width="360" alt="资料管理" />
</p>
...
### 评测结果（样例资料，18 题：可回答 14 / 无关 4）

**检索质量 · top_k 对比**

| top_k | hit@1 | hit@3 | 章节命中率 | 无关拒答率 | 误拒率 | 拒答准确率 |
|---|---|---|---|---|---|---|
| 1 | 85.7% | 85.7% | 50.0% | 100.0% | 0.0% | 100.0% |
| **3** | **85.7%** | **100.0%** | **85.7%** | **100.0%** | **0.0%** | **100.0%** |
| 5 | 85.7% | 100.0% | 100.0% | 100.0% | 0.0% | 100.0% |
| 8 | 85.7% | 100.0% | 100.0% | 100.0% | 0.0% | 100.0% |

`top_k=3`（默认）即可达到 hit@3=100%，再增大只小幅提升章节命中率、却拉长上下文，故取 3 为平衡点。

**拒答阈值标定**——阈值并非拍脑袋，而是由检索距离分布标定：

| 类型 | 最佳命中距离 min / 中位 / max |
|---|---|
| 可回答问题 | 0.614 / 0.752 / **0.862** |
| 无关问题 | **0.869** / 1.303 / 1.457 |

可回答问题的最大距离（0.862）与无关问题的最小距离（0.869）之间存在清晰间隔，故取阈值 **0.865** 落在两者之间：可回答问题**零误拒**、无关问题**全部拒答**（拒答准确率 100%）。该约束已固化为回归测试（`tests/test_rag.py`），改坏即报警。

> 说明：以上为内置样例资料上的结果，评测集规模有限，阈值标定与具体资料相关；换资料后需重跑评测重新标定。这是基于距离阈值的拒答方案的固有特性——当两类问题的距离分布重叠时，单靠阈值无法完美区分。
